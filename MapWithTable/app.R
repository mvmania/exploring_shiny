# Map with navigation bar and table 
...
Indonesia <- readOGR(dsn = "/Users/User/Desktop/R Prac/UNICEF Project/exploring_shiny/MapWithTable/shape", "IDN_adm1") # Shapefile 
...
server <- function(input, output){
        ...
        observe({
                ...
                output$SDGtable1 <- DT::renderDataTable({
                        DT::datatable(oo2 <- cbind.data.frame(SDG = oo$SDG , 
                                                              Indicator = oo$Indicator, 
                                                              Value = oo$Value, 
                                                              Province = oo$Province), 
                        options = list(
